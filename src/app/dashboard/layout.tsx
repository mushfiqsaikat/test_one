    ...
    const createChatbot = async () => {
        ...
        try {
            ...
            const { data: newBot } = await supabase
                .from("chatbots")
                .insert({ user_id: user.id, name: `Chatbot ${chatbots.length + 1}` })
                .select("id, name, status")
                .single();
    ...
    if (loading) {
        return (
            <div className="animate-pulse flex items-center gap-2 px-3 py-2">
                <div className="w-8 h-8 rounded-lg bg-slate-300 dark:bg-slate-600"></div>
                ...
                <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-slate-200 dark:border-slate-700 z-50 overflow-hidden">
                    ...
                    <div className="border-t border-slate-200 dark:border-slate-700">
                        <button
                            ...
                            disabled={creating}
                            className="flex items-center gap-2 px-3 py-2 w-full text-primary-600 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
                        >
                            {creating ? (
                            ...
                            <span className="text-sm font-medium">{creating ? "Creating..." : "Create new chatbot"}</span>
                        </button>
                    </div>
                </div>
            )}
