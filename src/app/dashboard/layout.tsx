            ...
            <header className="h-12 border-b border-slate-200 dark:border-slate-700 flex items-center px-4 bg-white dark:bg-slate-900 z-50">
                ...
                <div className="ml-auto flex items-center gap-2">
                    ...
                    <div className="relative ml-2">
                        <button
                            onClick={() => { setUserDropdownOpen(!userDropdownOpen); setWorkspaceDropdownOpen(false); setAgentDropdownOpen(false); }}
                            className="w-8 h-8 rounded-full bg-gradient-to-br from-orange-400 to-red-500 flex items-center justify-center text-white text-sm font-medium hover:ring-2 hover:ring-offset-2 hover:ring-primary-400 transition-all"
                        >
                            {userData?.email?.charAt(0).toUpperCase() || "U"}
                        </button>

                        {userDropdownOpen && (
                            <div className="absolute top-full right-0 mt-2 w-64 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 py-2 z-50">
                                {/* User Info */}
